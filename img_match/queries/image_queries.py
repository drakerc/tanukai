...
class ImageQueries:
    ...
    def save(
        ...
        **image_kwargs,
        ...
        if not status.OK():
            raise Exception(
                f"Could not add to Milvus because of an error: {status.message}."
            )
        image_id = ids[0]
        image_record = image_model(
            _id=image_id, image_path=image_path, timestamp=datetime.now()
        )
        ...
        result = image_record.save(
            using=self._elasticsearch.database, doc_type="_doc", refresh=refresh_index
        )
        if not result:
    ...
    def find(
        ...
        search_param = {"nprobe": 32}  # TODO: make it as a param
        ...
        if status.OK():
            ...
            for res in results[0][pagination_from:]:
                similar_results[str(res.id)] = {"distance": res.distance, "id": res.id}
                elastic_ids.append(res.id)
            elastic_search = Image.search(
                using=self._elasticsearch.database, index=config.elasticsearch_index
    ...
    def find_by_phash(
        ...
        response = elastic_search[
            pagination_from : pagination_from + pagination_size
        ].execute()
        for img in response:
            similar_results[img.meta.id] = {
    ...
    def find_by_id(
        ...
        status, vector = self._milvus.database.get_entity_by_id(
            collection_name=config.milvus_collection_name, ids=[image_id]
        )
    ...
    def create_milvus_collection(self):
        ...
        if status.OK():
            return True
        raise Exception(
            f"Could not create a collection because of an error: {status.message}."
        )
    ...
    def create_milvus_index(self):
        ...
        status = self._milvus.database.create_index(
            config.milvus_collection_name, IndexType.IVF_SQ8, index_param
        )
        if status.OK():
            return True
        raise Exception(
            f"Could not create an index because of an error: {status.message}."
        )

    def create_milvus_partition(self, partition_tag: str) -> bool:
        status = self._milvus.database.create_partition(
            collection_name=config.milvus_collection_name, partition_tag=partition_tag
        )
        if status.OK():
            return True
        raise Exception(
            f"Could not create a partition because of an error: {status.message}."
        )
    ...
    def get_partitions(self) -> dict:
        ...
        """
        status, stats = self._milvus.database.get_collection_stats(
            config.milvus_collection_name
        )
        return {
            i["tag"]: i["row_count"] for i in stats["partitions"] if i["row_count"] > 0
        }
    ...
    def milvus_collection_exists(self) -> bool:
        status, has_collection = self._milvus.database.has_collection(
            config.milvus_collection_name
        )
        return has_collection

    def milvus_partition_exists(self, partition_tag: str) -> bool:
        status, has_partition = self._milvus.database.has_partition(
            config.milvus_collection_name, partition_tag
        )
