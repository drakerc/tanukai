...
class TanukaiImageQueries(ImageQueries):
    ...
    def remove_duplicates(self):
        ...
        for number, result in enumerate(elastic_search.scan()):
            ...
            if len(response) > 1:
                ...
                status = self._milvus.database.delete_entity_by_id(
                    config.milvus_collection_name, [int(img_to_delete_id)]
                )
                if not status.OK():
                    raise Exception(
    ...
    def move_to_partition(self):
        ...
        status = self._milvus.database.create_partition(
            collection_name="images", partition_tag="e621"
        )
        elastic_search = Image.search(
            using=self._elasticsearch.database, index=config.elasticsearch_index
        )
        for number, result in enumerate(elastic_search.scan()):
            ...
            status, vectors = self._milvus.database.get_entity_by_id(
                config.milvus_collection_name, [int(image_id)]
            )
            status, ids = self._milvus.database.insert(
                collection_name="images", partition_tag="e621", records=vectors
            )
            ...
            new_image_result = result_with_new_id.save(
                using=self._elasticsearch.database, doc_type="_doc", refresh=False
            )
    ...
    def rename_source_website(self):
        ...
        for number, result in enumerate(elastic_search.scan()):
            print(f"Processing {number}")
            result.update(
                using=self._elasticsearch.database, refresh=False, source_website="e621"
            )
    ...
    def _add_partitions_to_cache(self, partitions: dict) -> dict:
        ...
        self._redis.database.expire(
            self.REDIS_PARTITIONS_KEY, self.REDIS_PARTITIONS_EXPIRATION_TIME
        )
        return partitions
