...
class DanbooruScraper(scrapy.Spider):
    ...
    def start_requests(self):
        headers = {"User-Agent": "ImgSearch (drakeapp@gmail.com)"}
        query = {
            "limit": 120,
        }
        ...
        yield scrapy.Request(
            url=url, callback=self.parse, headers=headers, dont_filter=True
        )
    ...
    def parse(self, response):
        ...
        for data in posts:
            ...
            es_tags = {"general": tags, "artist": authors}
        ...
        if not self.state.get("first_id") and "page" not in response.url:
            self.state[
                "first_id"
            ] = first_post_id  # set the highest ID on the first scrape ever

        headers = {"User-Agent": "ImgSearch (drakeapp@gmail.com)"}
        for i in reversed(posts):
            item_id = i.get("id")
            if item_id:
                query = {"limit": 120, "page": f"b{item_id}"}
                encoded_query = urlencode(query)
                url = f"https://danbooru.donmai.us/posts.json?{encoded_query}"
